...
class Output:
    ...
    def disable(self):
        ...
        #new: set number of crtc to zero (consistency - may have caused trouble when defining whether or not output has changed) 
        self._info.contents.crtc = 0
        self._crtc = None
...
class Crtc:
    ...
    def disable(self):
        ...
        # changes for consistency: has caused trouble defining whether or not Crtc changed and finding the right position
        self._info.contents.x=0
        self._info.contents.y=0
    ...
    def add_output(self, output):
        ...
        # for consistency= store changes to output and reset the output crtc when a new one is added
        output._info.contents.crtc = self.xid
...
class Screen:
    ...
    def _arrange_outputs(self):
        ...
        for output in self.get_outputs():
            ...
            if not output.has_changed(CHANGES_RELATION) or output._mode == None:
                continue
            else:
    ...
    def _calculate_size(self):
        ...
        # all comments may be a possible solution for the calculation of the physical size. Failed until now, maybe due to a premature call of XRRSetScreenSize   
        width = self._width
    #width=0
        height = self._height
    #height=0
    ...
    #biggerH = False
        ...
        #biggerH = True
        #if biggerW and biggerH:
        #continue
        #if not biggerW:
        #width= x+w
        #if not biggerH:
        #height= y+h
    #phys_height=(height/6.8)+20
    #phys_width=(width/6.8)+20
        if width > self._width_max or height > self._height_max:
            ...
            if height < self._height_min: 
                ...
                self._height = height
        #self._height_mm=phys_height
            if width < self._width_min: 
                self._width = self._width_min
            else:
                self._width = width
        #self._width_mm=phys_width
